...
def display_instances(image, boxes, masks, class_ids, class_names,
                      scores=None, title="",
                      figsize=(16, 16), ax=None, save_dir=None, image_name=None):
    """
    ...
    if save_dir is None:
        save_dir = './tmp'
        image_name = 'tmp.jpg'
    cv2.imwrite(os.path.join(save_dir, image_name), masked_image.astype(np.uint8))
    print('save to {}/{}'.format(save_dir, image_name))
...
def display_keypoints_and_instance(image, boxes, keypoints, masks, class_ids, class_names,
                      skeleton = [], scores=None, title="",
                      figsize=(16, 16), ax=None, save_dir=None,
                      image_name=None):
    """
    ...
    masked_image = skeleton_image.astype(np.uint32).copy()
    for i in range(N):
        color = colors[i]
...
def display_keypoints(image, boxes, keypoints, class_ids, class_names,
    ...
    if save_dir is None:
        save_dir = './tmp'
        image_name = 'tmp.jpg'
    cv2.imwrite(os.path.join(save_dir, image_name), skeleton_image)
    print('save to {}/{}'.format(save_dir, image_name))
